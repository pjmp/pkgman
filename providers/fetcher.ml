...
let rec get ?(download = false) ?(redirects = 5) url =
  let headers =
    [|
  ...
  follow_redirect ~redirects url ans

and follow_redirect ~redirects request_uri (response, body) =
  let open Lwt.Syntax in
...
and handle_redirect ~permanent ~redirects request_uri response =
  if redirects <= 0 then Lwt.fail_with "Too many redirects"
  else
    let location =
